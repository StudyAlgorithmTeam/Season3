...
class Grid:
    def __init__(self, width: int, height: int):
        ...
        self.grid_buffer = [[0]*width for _ in range(height)]
    ...
    def simulate(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                self.grid_buffer[y][x] = self.grid[y][x]
                for ny, nx in self.neighbors_of(y, x):
                    if self.grid_buffer[y][x] > 0 and self.grid[ny][nx] == MELTED:
                        self.grid_buffer[y][x] -= 1
        self.grid, self.grid_buffer = self.grid_buffer, self.grid
...
if __name__ == "__main__":
    ...
    # 100x100 격자로 빙산을 이룰때, 약 500년(50칸 x 높이 10개)이 걸릴 수도 있다.
    steps = 0
